...
標準デモでは公式 Unity-chan 3D Model Data v1.4.0 のHumanoid FBXを、G1の右側へ2 m離して表示します。両者は同じ受信ルート姿勢を使い、比較用オフセットを保ったまま移動します。付属アニメーションや旧入力スクリプトは使用せず、MotionBricksが生成した姿勢だけを適用します。モデルが欠落している場合に限り、テスト用の手続き生成Humanoidへフォールバックします。
