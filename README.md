...
- Unity-chan: 同じ G1 姿勢を Unity Humanoid muscle へ変換した比較表示
...
`G1HumanoidRetargeter` は G1 の脚12軸、腰3軸、腕14軸を Unity Humanoid muscle へ対応付けます。関節クォータニオンから角度を推定せず、Python側のMJCF `qpos` に含まれる正確なヒンジ角を使用します。任意のキャラクターで利用する場合は、Humanoid として正しく Import された有効な `Animator`／`Avatar` を割り当てます。無効な Avatar は例外を出さず拒否されます。

標準デモでは公式 Unity-chan 3D Model Data v1.4.0 のHumanoid FBXをシーンへ直接配置し、G1の右側へ2 m離して表示します。両者は同じ受信ルート姿勢を使い、比較用オフセットを保ったまま移動します。付属アニメーションや旧入力スクリプトは使用せず、MotionBricksが生成した姿勢だけを適用します。
